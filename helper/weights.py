...
def lumi_weight(rdf: Any, era: str) -> Any:
    ...
    if era == "2016preVFP":
        ...
        rdf = rdf.Redefine("weight", "weight * 108.83 * 1000.")
    else:
...
def apply_btag_weight(rdf: Any) -> Any:
    ...
    """
    rdf = rdf.Define("wgt_with_btag", "weight * btag_weight_upart")
    ...
    # h = rdf.Histo1D(("", "", nbinsx, xbinning), "n_jets", "weight")
    # h = h.GetValue()
    # # histogram with b-tagging SFs
    # h_btag = rdf.Histo1D(("", "", nbinsx, xbinning), "n_jets", "wgt_with_btag")
    ...
    # btag_wgt = "btag_weight_upart*("
    # for n in range(nbinsx):
    #     if ratio.GetBinContent(n + 1) != 0.0:
    #         btag_wgt += "(n_jets=={})*{}+".format(n, ratio.GetBinContent(n + 1))
    #     else:
    #         btag_wgt += "(n_jets=={})*{}+".format(n, 1.0)
    # btag_wgt += "(n_jets>8)*1.)"
