...
def get_split_combinations(categories: Dict[str, List[str]]) -> Tuple[List[str], List[Dict[str, str]]]:
    '''
...
def apply_region_filters(rdf: Any, channel: str, sample: str, category_cuts: Union[Dict[str, str], None], region_cuts: Dict[str, str]) -> Any:
    '''
        ...
        if cut != "nbtag":
            if "had_tau_id_vs_jet" in cut:
                ...
                if sample not in ["data"]:
                    rdf = weights.apply_tau_id_vsJet_weight(rdf=rdf, channel=channel, wps=wps, idx=idx)
                rdf = rdf.Filter(f"({sum_cuts[cut]})", f"cut on {cut}")

            elif "had_boostedtau_id_iso" in cut: # this is only relevant for an analysis with boosted tau pairs
                ...
                try:
                    ...
                    idx = None
                if sample not in ["data"]:
                    rdf = weights.apply_boostedtau_id_iso_weight(rdf=rdf, channel=channel, cut_string=sum_cuts[cut], wp=wp, idx=idx)
                rdf = rdf.Filter(f"({sum_cuts[cut]})", f"cut on {cut}")
...
def calculate_QCD_FF(SRlike: Dict[str, Any], ARlike: Dict[str, Any]) -> Tuple[Any, Any, Any]:
    '''
...
def calculate_Wjets_FF(SRlike: Dict[str, Any], ARlike: Dict[str, Any]) -> Tuple[Any, Any, Any]:
    '''
    Function which calculates fake factors based on the histograms from the determination regions.
    Additionally up and down variations are calculated for the MC subtraction.
...
def calculate_ttbar_FF(SR: Dict[str, Any], AR: Dict[str, Any], SRlike: Dict[str, Any], ARlike: Dict[str, Any]) -> Any:
    '''
...
def add_fraction_variations(hists: Dict[str, Any], processes: List[str]) -> Dict[str, Dict[str, Any]]:
    '''
...
def get_yields_from_hists(hists: Dict[str, Dict[str, Any]], processes: List[str]) -> Dict[str, Dict[str, Dict[str, List[float]]]]:
    '''
...
def fit_function(ff_hists: Union[List[Any], Any], bin_edges: List[int]) -> Tuple[Dict[str, Any], Dict[str, str]]:
    '''
    ...
    corrlib_expressions["slope_unc_up"] = f"{(fit.Parameter(1) + fit.ParError(1))}*x+{fit.Parameter(0)}"
    corrlib_expressions["slope_unc_down"] = f"{(fit.Parameter(1) - fit.ParError(1))}*x+{fit.Parameter(0)}"
    # normalization parameter up/down
    corrlib_expressions["normalization_unc_up"] = f"{fit.Parameter(1)}*x+{(fit.Parameter(0) + fit.ParError(0))}"
    corrlib_expressions["normalization_unc_down"] = f"{fit.Parameter(1)}*x+{(fit.Parameter(0) - fit.ParError(0))}"
    # MC subtraction uncertainty up/down
    if do_mc_subtr_unc:
        corrlib_expressions["mc_subtraction_unc_up"] = f"{fit_up.Parameter(1)}*x+{fit_up.Parameter(0)}"
        corrlib_expressions["mc_subtraction_unc_down"] = f"{fit_down.Parameter(1)}*x+{fit_down.Parameter(0)}"
